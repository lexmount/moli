...
impl JsContextHost {
    pub(in crate::native_bridge) fn begin_child_document_stream_replacement<'s>(
        ...
        if self.child_document_active_parser_was_aborted(child_handle) {
            tracing::debug!(
                ?child_handle,
                ?document_handle,
        ...
        let has_ongoing_navigation = self
            .child_browsing_contexts
            .get(&child_handle)
            .is_some_and(|entry| entry.has_pending_navigation_or_document_load())
            && !self.child_browsing_context_has_pending_cross_document_traversal(child_handle);
        if has_ongoing_navigation {
            let window = script_context.global(scope);
            if let Some(owner) =
                self.current_window_execution_context_owner(OwnerDispatchScope::Child(child_handle))
                && let Some(binding) = self.clone_window_execution_context_binding(
                    scope,
                    owner,
            ...
            if let Some(owner) = self.current_child_document_task_owner(child_handle) {
                self.abort_window_requests_for_document(
        ...
        if self.child_browsing_context_document_handle(child_handle) != Some(document_handle)
            || self.child_document_active_parser_was_aborted(child_handle)
        {
            return None;
        }
