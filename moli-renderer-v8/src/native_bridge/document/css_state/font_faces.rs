...
pub(crate) fn synchronize_font_face_set_load_state_for_attribute<'s>(
    ...
    let Ok((runtime_ptr, document_handle)) = node_runtime_and_handle_from_object(scope, document)
    else {
        return;
    };
    ...
    let cycle = if request_ready_layout {
        runtime.reserve_document_web_font_ready_cycle()
    } else {
    ...
    if request_ready_layout {
        runtime.queue_document_web_font_ready_rendering_update_if_needed();
    }
}
