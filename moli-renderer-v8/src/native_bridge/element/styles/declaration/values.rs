...
fn resolved_layout_box_dimension(
    ...
    .ok()??;
    let Some(size) = metrics.resolved_size else {
        // The frozen tree has geometry for this source but no CSS box used
        // value (for example, a non-replaced inline). Do not reconstruct one
        // from declarations and ancestor sizes.
