...
/// Projects one native document-font loading cycle into the stable
/// `FontFaceSet.ready` promise owned by the document wrapper.
...
/// Resolves the exact cycle only after an explicit fresh layout consumed all
/// terminal registrations. A later cycle cannot accidentally settle an older
/// promise because the native and JS identities must agree.
