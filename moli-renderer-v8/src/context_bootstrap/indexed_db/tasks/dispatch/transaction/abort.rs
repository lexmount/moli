...
use super::*;
use crate::context_bootstrap::indexed_db::take_indexed_db_aborted_open;
...
pub(in crate::context_bootstrap::indexed_db) fn flush_transaction_abort_task<'s>(
    ...
    let aborted_open = take_indexed_db_aborted_open(scope, transaction);
    ...
    if let Some((request, _)) = aborted_open {
        finish_aborted_upgrade_open(scope, request);
    }
}
