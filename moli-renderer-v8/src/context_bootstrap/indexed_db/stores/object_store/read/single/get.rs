...
#[webidl(prefix = "IDBObjectStore.get")]
struct IdbObjectStoreGetArgs<'s> {
    #[webidl(required, converter = "raw")]
    query: v8::Local<'s, v8::Value>,
}

pub(in crate::context_bootstrap::indexed_db) fn idb_object_store_get_callback<'s>(
    scope: &mut v8::PinScope<'s, '_>,
    args: v8::FunctionCallbackArguments<'s>,
    mut rv: v8::ReturnValue<'s, v8::Value>,
) {
    let Some(parsed) = webidl::parse_args::<IdbObjectStoreGetArgs<'s>>(scope, &args) else {
        return;
    };
    let store = args.this();
        ...
        .unwrap_or(false)
    {
        enqueue_transaction_operation(
            ...
            request,
            &store_name,
            IndexedDbTransactionOperationInput::ObjectStoreGet {
                query: parsed.query,
            },
        );
        rv.set(request.into());
