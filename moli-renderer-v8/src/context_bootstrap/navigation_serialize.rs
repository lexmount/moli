...
pub(super) fn sync_navigation_entry_seed_from_owner<'s>(
    ...
    sync_navigation_entry_seed_from_owner_with_document_url(
        scope,
        owner,
        true,
...
pub(super) fn sync_child_pending_navigation_entry_seed_from_owner<'s>(
    ...
    sync_navigation_entry_seed_from_owner_with_document_url(scope, owner, false, commit);
}
...
pub(super) fn serialize_navigation_entry_object<'s>(
    ...
    if let Some(snapshot) = history_entries
        .iter()
        .find(|snapshot| snapshot.id.as_str() == record.id && snapshot.key == record.key)
    {
        return snapshot.clone();
    }
    snapshot_native_entry(&record, record.index)
}

fn snapshot_native_entry(
    ...
    NavigationHistorySerializedEntry {
        ...
        history_index,
        index: entry.index,
        id: NavigationHistoryEntryId::from_serialized(entry.id.clone()),
        key: entry.key.clone(),
    }
}
