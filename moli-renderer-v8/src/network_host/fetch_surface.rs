...
fn response_clone_callback<'s>(
    ...
    if let Some(clone) = ctor.new_instance(scope, &[body, init.into()]) {
        if let Some(url) = response_slot_value(scope, this, RESPONSE_URL_SLOT) {
            set_response_slot_value(scope, clone, RESPONSE_URL_SLOT, url);
        }
        rv.set(clone.into());
    } else {
        rv.set_undefined();
...
fn install_body_stream_consumer_runtime(scope: &mut v8::PinScope<'_, '_>) -> anyhow::Result<()> {
    ...
    let initializer = crate::script_execution::execute_compiled_script(scope, script)
        .and_then(|value| v8::Local::<v8::Function>::try_from(value).ok())
        .ok_or_else(|| anyhow::anyhow!("failed to create Fetch body stream consumer runtime"))?;
    crate::script_execution::call_function(
        scope,
        initializer,
        v8::undefined(scope).into(),
        &[get_reader.into(), read.into()],
    )
    .ok_or_else(|| anyhow::anyhow!("failed to initialize Fetch body stream consumer runtime"))?;
    Ok(())
}
