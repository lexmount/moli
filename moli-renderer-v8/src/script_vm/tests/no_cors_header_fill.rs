...
async fn no_cors_header_fill_uses_append_guards_in_window_and_worker() {
    ...
    for worker in [false, true] {
        ...
        let mut vm =
            new_page_task_executor_test_vm_with_loader("https://no-cors-fill.test/", &loader);
        vm.eval("globalThis.headerFillResult = null;").unwrap();
        let script = if worker {
            let source = format!(
                "{fixture}\nnoCorsFillProbe('https://no-cors-fill.test', false).then(postMessage, error => postMessage({{error: String(error.stack || error)}}));"
            );
            format!(
                r#"
            ...
            format!(
                "{fixture}\nnoCorsFillProbe('https://no-cors-fill.test', false).then(value => {{ headerFillResult = value; }}, error => {{ headerFillResult = {{error: String(error.stack || error)}}; }});"
            )
        };
        ...
        assert_eq!(checks.len(), 234, "worker={worker}");
    }
}
