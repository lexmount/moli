...
#[test]
fn webgl_unavailable_creation_matches_chromium_without_a_gpu_backend() {
    let mut vm = new_storage_test_vm("https://webgl-unavailable.test/");
    let result = vm
        .eval(include_str!("../../../tests/fixtures/webgl-unavailable.js"))
        .expect("WebGL creation failure, receiver and fallback contracts should hold");
    assert_eq!(
        result,
        r#"["html:webgl","html:experimental-webgl","html:webgl2","offscreen:webgl","offscreen:webgl2"]"#
    );
}

#[test]
fn webgl_creation_failure_preserves_reentrant_fallback_and_native_event_construction() {
    let mut vm = new_storage_test_vm("https://webgl-reentrancy.test/");
    let result = vm.eval(r#"
      (() => {
        const NativeEvent = WebGLContextEvent;
        const canvas = document.createElement('canvas');
        let calls = 0, context, nativeEvent = false;
        canvas.addEventListener('webglcontextcreationerror', event => {
          calls++;
          nativeEvent = event instanceof NativeEvent && event.isTrusted;
          context = canvas.getContext('2d');
          throw new Error('listener exception must not escape getContext');
        });
        globalThis.WebGLContextEvent = () => { throw new Error('author constructor must not be called'); };
        const result = canvas.getContext('webgl');
        return [result === null, calls, nativeEvent, !!context, canvas.getContext('2d') === context].join('|');
      })()
    "#).expect("failure listener exceptions must be reported, not propagate to the factory");
    assert_eq!(result, "true|1|true|true|true");
}
