...
#[test]
fn location_components_child_getters_preserve_committed_urls() {
    for (suffix, search, hash) in [
        ("", "", ""),
        ("?", "", ""),
        ("#", "", ""),
        ("?#", "", ""),
        ...
        let href = format!("about:blank{suffix}");
        vm.eval(&format!(
            "const child = document.createElement('iframe'); child.id = 'child'; \
             child.src = {href:?}; document.body.appendChild(child);"
        ))
        .expect("child URL setup should evaluate");
        vm.drain_pending_child_frame_work_for_test();
        let observed = vm
            .eval(
                "(() => { const child = document.getElementById('child'); \
                 const location = child.contentWindow.location; \
                 return JSON.stringify([location.search, location.hash, location.href, \
                 child.contentDocument.URL]); })()",
            )
            .expect("committed child Location getters should evaluate");
        assert_eq!(
            observed,
            serde_json::json!([search, hash, href, href]).to_string(),
            "child URL {href}"
        );
    }
}
