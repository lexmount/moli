...
#fixed{position:fixed;left:300px;top:40px}
#spacer{height:800px}
</style>`;
...
"#,
        ...
        let result = page_vm.vm_mut().eval(
            r#"JSON.stringify((()=>{
const ids=['no-zoom','x2-zoom','x4-relative','x8-effective','transformed','ancestor-transformed','fixed','negative'];
...
return {
  ...
  wrongReceiver
        ...
        page_vm.vm_mut().eval("scrollTo(0, 100); 'scrolled'")?;
        page_vm
            .vm_mut()
            .screenshot_layout_snapshot(moli_layout::PaintViewport::new(480, 320, 1.0))?
            .expect("scrolled image-coordinate fixture must retain a layout root");
        let scrolled = page_vm.vm_mut().eval(
            r#"JSON.stringify((()=>{
const geometry=id=>{const image=document.getElementById(id);const rect=image.getBoundingClientRect();return [image.x,image.y,rect.left,rect.top]};
return {scrollY,flow:geometry('ancestor-transformed'),fixed:geometry('fixed')};
})())"#,
        )?;
        let scrolled: serde_json::Value = serde_json::from_str(&scrolled)?;
        assert_eq!(scrolled["scrollY"], 100);
        assert_eq!(scrolled["flow"], serde_json::json!([220, -80, 260, -50]));
        assert_eq!(scrolled["fixed"], serde_json::json!([300, 40, 300, 40]));
