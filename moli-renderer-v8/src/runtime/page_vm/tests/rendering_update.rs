...
#[tokio::test(flavor = "current_thread")]
async fn atomic_inline_keeps_the_parent_inline_subpixel_origin() {
    run_page_vm_async_test(async move {
        let loader =
            crate::network::ResourceRequestClient::new(&FetchConfig::default()).expect("loader");
        let mut page_vm = test_page_vm_with_loader_and_document_url(
            &loader,
            Vec::new(),
            Url::parse("https://example.com/atomic-inline-subpixel-origin.html")?,
        );
        page_vm.vm_mut().eval(
            r#"
document.head.innerHTML = `<style>
...
"#,
        ...
        let inline_unit_epsilon = 2.0 / 65_536.0;

        assert!(
            (preceding_right - preceding_right.round()).abs() > 0.05,
            "fixture must end its preceding text at a meaningful subpixel origin: {geometry}"
        );
        assert!(
            (atomic_x - preceding_right).abs() <= inline_unit_epsilon,
            "the atomic outer placement must retain the parent IFC's subpixel origin: {geometry}"
        );
        assert!(
            (text_x - atomic_x).abs() <= inline_unit_epsilon,
            "the atomic IFC must paint its first glyph at the same subpixel origin: {geometry}"
        );
        Ok::<_, anyhow::Error>(())
    })
    .await
    .expect("atomic inline subpixel fixture should run");
}
