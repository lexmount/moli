...
async fn screenshot_preserves_ratio_dependent_automatic_inline_minimums() {
    run_page_vm_async_test(async move {
        let loader = crate::network::ResourceRequestClient::new(&FetchConfig::default())?;
        let mut page = test_page_vm_with_loader_and_document_url(
            &loader,
            Vec::new(),
            Url::parse("https://example.com/ratio-automatic-inline-minimum.html")?,
        );
        page.vm_mut()
            .set_layout_policy(moli_page_types::LayoutPolicy::OnDemand);
        let fixture =
            include_str!("../../../../tests/fixtures/aspect-ratio-automatic-inline-minimum.html");
        ...
        // Synchronous reads observe the last snapshot; demand layout explicitly.
        page.vm_mut()
            .screenshot_layout_snapshot(moli_layout::PaintViewport::new(1280, 4000, 1.0))?
            .expect("ratio minimum fixture must retain a layout root");
        let checks: serde_json::Value = serde_json::from_str(
            &page
                .vm_mut()
                .eval("JSON.stringify(collectRatioMinimumChecks())")?,
        )?;
        let checks = checks.as_array().expect("ratio minimum checks");
        assert_eq!(checks.len(), 153);
        for check in checks {
    ...
    .await
    .expect("ratio automatic inline minimum fixture should run");
}
