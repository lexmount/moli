...
"#,
        ...
        assert_eq!(
            page_vm.vm_mut().eval(
                r#"JSON.stringify({computed:Object.fromEntries(['precedence','content','border','atomic','inline','hidden','contents','stale','zoom'].map(id=>{const style=getComputedStyle(document.getElementById(id));return [id,[style.width,style.height]]})),rects:Object.fromEntries(['precedence','content','border','atomic','stale','zoom'].map(id=>{const rect=document.getElementById(id).getBoundingClientRect();return [id,[rect.width,rect.height]]}))})"#,
            )?,
            r#"{"computed":{"precedence":["260px","0px"],"content":["100px","40px"],"border":["100px","40px"],"atomic":["120px","30px"],"inline":["80%","30px"],"hidden":["77%","88%"],"contents":["66%","55%"],"stale":["90px","45px"],"zoom":["100px","20px"]},"rects":{"precedence":[260,0],"content":[260,140],"border":[100,40],"atomic":[120,30],"stale":[120,75],"zoom":[240,80]}}"#,
