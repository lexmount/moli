...
"#,
    ...
    #[test]
    fn layout_renderer_resolves_css_paragraph_and_inline_object_directions() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("current-thread runtime should build");
        runtime.block_on(tokio::task::LocalSet::new().run_until(async move {
            let mut page = parse_phase_one_html_into_page_vm_for_test_with_env(
                include_str!("../../../tests/fixtures/inline-bidi-objects.html"),
                default_test_page_vm_env_config_with(|env| {
                    env.layout_policy = moli_page_types::LayoutPolicy::OnDemand;
                }),
            )
            .await;
            // Exercise the initial build and rebuilding an existing IFC in
            // both directions. Only explicit captures publish new geometry.
            for direction in ["rtl", "ltr", "rtl"] {
                page.vm_mut()
                    .eval(&format!(
                        "{{ const slot = document.getElementById('dynamic-direction'); \
                         slot.style.direction = '{direction}'; \
                         slot.dataset.expected = '{}'; }}",
                        if direction == "rtl" {
                            "150,0,20,10"
                        } else {
                            "30,0,20,10"
                    ...
                    .expect("update paragraph direction");
                page.vm_mut().sync_live_document_style_sources();
                page.vm_mut()
                    .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))
                    .expect("bidi fixture layout should succeed")
                    .expect("fixture should have a document element");
                let geometry = page
                    .vm_mut()
                    .eval(
...
"#,
                    ...
                    .expect("read published bidi geometry");
                ...
                for case in cases {
                    for axis in 0..4 {
                        ...
                        assert!(
                            (actual - expected).abs() <= 1.0 / 64.0,
                            "{}[{axis}] after {direction}: expected {expected}, got {actual}",
                            case["id"],
                        );
                    }
                }
    ...
    #[test]
    fn layout_renderer_preserves_text_flow_across_out_of_flow_placeholders() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("current-thread runtime should build");
        runtime.block_on(tokio::task::LocalSet::new().run_until(async move {
            let mut page = parse_phase_one_html_into_page_vm_for_test_with_env(
                include_str!("../../../tests/fixtures/inline-out-of-flow-line-breaking.html"),
                default_test_page_vm_env_config_with(|env| {
                    env.layout_policy = moli_page_types::LayoutPolicy::OnDemand;
                }),
            )
            ...
            for font_size in [20, 32] {
                page.vm_mut()
                    .eval(&format!("document.body.style.fontSize = '{font_size}px'"))
                    .expect("change the line-breaking fixture's font size");
                page.vm_mut().sync_live_document_style_sources();
                page.vm_mut()
                    .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))
                    .expect("line-breaking layout should succeed")
                    .expect("fixture should have a document element");
                let result = page
                    .vm_mut()
                    .eval("JSON.stringify(collectInlineFragmentChecks())")
                ...
                let checks: serde_json::Value =
                    serde_json::from_str(&result).expect("geometry JSON");
                let checks = checks.as_array().expect("line-breaking checks");
                assert_eq!(checks.len(), 192);
                for check in checks {
                    assert_eq!(
                        check["actual"], check["expected"],
                        "{} with font size {font_size}",
    ...
    #[test]
    fn layout_renderer_keeps_non_atomic_inline_edges_with_their_contents() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("current-thread runtime should build");
        runtime.block_on(tokio::task::LocalSet::new().run_until(async move {
            let mut page = parse_phase_one_html_into_page_vm_for_test_with_env(
                include_str!("../../../tests/fixtures/inline-boundary-layout.html"),
                default_test_page_vm_env_config_with(|env| {
                    env.layout_policy = moli_page_types::LayoutPolicy::OnDemand;
                }),
            )
            ...
            for font_size in [20, 32] {
                page.vm_mut()
                    .eval(&format!("document.body.style.fontSize = '{font_size}px'"))
                    .expect("change the inline fixture's font size");
                page.vm_mut().sync_live_document_style_sources();
                page.vm_mut()
                    .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))
                    .expect("inline boundary layout should succeed")
                    .expect("fixture should have a document element");
                let result = page
                    .vm_mut()
                    .eval("JSON.stringify(collectInlineFragmentChecks())")
                ...
                let checks: serde_json::Value =
                    serde_json::from_str(&result).expect("geometry JSON");
                let checks = checks.as_array().expect("inline boundary checks");
    ...
    #[test]
    fn layout_renderer_collapses_line_edge_geometry_without_losing_source_positions() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("current-thread runtime should build");
        runtime.block_on(tokio::task::LocalSet::new().run_until(async move {
            let mut page = parse_phase_one_html_into_page_vm_for_test_with_env(
                include_str!("../../../tests/fixtures/inline-line-edge-whitespace.html"),
                default_test_page_vm_env_config_with(|env| {
                    env.layout_policy = moli_page_types::LayoutPolicy::OnDemand;
                }),
            )
            ...
            for font_size in [20, 32] {
                page.vm_mut()
                    .eval(&format!("document.body.style.fontSize = '{font_size}px'"))
                    .expect("change the line-edge fixture's font size");
                page.vm_mut().sync_live_document_style_sources();
                page.vm_mut()
                    .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))
                    .expect("line-edge layout should succeed")
                    .expect("fixture document element");
                let result = page
                    .vm_mut()
                    .eval("JSON.stringify(collectInlineFragmentChecks())")
                    .expect("read explicitly published line geometry");
                let checks: serde_json::Value =
                    serde_json::from_str(&result).expect("geometry JSON");
                let checks = checks.as_array().expect("line-edge checks");
    ...
    #[test]
    fn layout_renderer_defers_inline_floats_until_the_originating_line_finishes() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .expect("current-thread runtime should build");
        runtime.block_on(tokio::task::LocalSet::new().run_until(async move {
            let mut page = parse_phase_one_html_into_page_vm_for_test_with_env(
                include_str!("../../../tests/fixtures/inline-deferred-floats.html"),
                default_test_page_vm_env_config_with(|env| {
                    env.layout_policy = moli_page_types::LayoutPolicy::OnDemand;
                }),
            )
            ...
            for font_size in [20, 32] {
                page.vm_mut()
                    .eval(&format!("document.body.style.fontSize = '{font_size}px'"))
                    .expect("change the float fixture's font size");
                page.vm_mut().sync_live_document_style_sources();
                page.vm_mut()
                    .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))
                    .expect("inline float layout should succeed")
                    .expect("fixture document element");
                let result = page
                    .vm_mut()
                    .eval("JSON.stringify(collectInlineFragmentChecks())")
                    .expect("read explicitly published float geometry");
                let checks: serde_json::Value =
                    serde_json::from_str(&result).expect("geometry JSON");
                let checks = checks.as_array().expect("inline float checks");
                assert_eq!(checks.len(), 68);
                for check in checks {
                    assert_eq!(
                        check["actual"], check["expected"],
                        "{check} at {font_size}px"
                    );
                }
            }
        }));
