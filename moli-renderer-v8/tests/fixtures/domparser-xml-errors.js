function domParserXmlErrorsProbe() {
  ...
  function metadata(doc, mime, label, realm) {
    ...
    check(label + '/not-XMLDocument', doc instanceof realm.XMLDocument, false);
    check(label + '/prototype', Object.getPrototypeOf(doc) === realm.Document.prototype, true);
    check(label + '/URL', doc.URL, realm.document.URL);
    check(label + '/documentURI', doc.documentURI, realm.document.URL);
    check(label + '/baseURI', doc.baseURI, realm.document.URL);
    ...
    check(label + '/readyState', doc.readyState, 'complete');
    check(label + '/defaultView', doc.defaultView, null);
    check(label + '/location', doc.location, null);
    check(label + '/hidden', doc.hidden, true);
    check(label + '/visibilityState', doc.visibilityState, 'hidden');
    check(label + '/compatMode', doc.compatMode, 'CSS1Compat');
  }
