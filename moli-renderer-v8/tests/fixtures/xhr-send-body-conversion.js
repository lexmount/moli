function xhrSendBodyConversionProbe() {
  ...
  let nextUrlId = 0;
  const nextDataUrl = () => 'data:text/plain,complete-' + nextUrlId++;
  ...
  for (const kind of ['fixed', 'detached']) {
    ...
    for (const [index, body] of [buffer, view, dataView].entries()) {
      const xhr = new XMLHttpRequest(); xhr.open('POST', nextDataUrl(), false);
  ...
  const retry = new XMLHttpRequest(); retry.open('POST', nextDataUrl(), false);
  let rejected = false;
