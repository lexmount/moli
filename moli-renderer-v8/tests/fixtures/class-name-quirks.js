function classNameQuirksProbe(mainQuirks) {
  ...
  const quirks = parser.parseFromString(html, 'text/html');
  ...
  for (const [doc, label, insensitive] of [
    [document, 'live', mainQuirks], [quirks, 'parsed-quirks', true],
  ...
  try {
    ...
    const foreign = new other.DOMParser().parseFromString(html, 'text/html');
