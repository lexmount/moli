...
async fn websocket_failed_and_synthetic_delivery_cancel_on_last_handle_drop() {
    for synthetic in [false, true] {
        ...
        let handle = if synthetic {
            spawn_synthetic_connection(63, Vec::new().into(), 101, Vec::new().into(), sink).0
        } else {
...
async fn websocket_synthetic_connection_opens_accounts_send_and_closes_cleanly() {
    ...
    let (command_tx, _peer) = spawn_synthetic_connection(
        92,
        vec![("Origin".to_owned(), "http://example.test".to_owned())].into(),
        101,
        vec![("Sec-WebSocket-Protocol".to_owned(), "chat".to_owned())].into(),
        event_tx,
    );
...
async fn websocket_synthetic_peer_does_not_keep_client_alive() {
    let (events, mut receiver) = mpsc::channel(4);
    let (connection, peer) =
        spawn_synthetic_connection(96, Vec::new().into(), 101, Vec::new().into(), events);
...
async fn websocket_native_handshake_generates_keys_and_reports_actual_headers() {
    ...
    for socket_id in 98..100 {
        ...
        context
            .extra_headers
            .push(("Sec-WebSocket-Key".to_owned(), b"injected".to_vec()));
            ...
            .unwrap()
        {
            Event::Open {
                ...
                assert_eq!(
                    header_value(&request_headers.to_byte_strings(), "sec-websocket-key"),
                    Some(key)
                );
                assert_eq!(
                    header_value(&request_headers.to_byte_strings(), "host"),
                    header_value(&actual, "host")
                );
                assert_eq!(
                    header_value(&request_headers.to_byte_strings(), "sec-websocket-version")
                        .as_deref(),
                    Some("13")
                );
            }
