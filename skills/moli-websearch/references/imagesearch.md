...
Choose a URL method for the relevant index from the engine table. Encode the
image URL once as the `ENC` query value:

```bash
moli fetch --timeout 28000 --wait-until done --dump markdown "$SEARCH_URL"
```

An outer `timeout 35s` can bound the process. If only an empty image and “No
matching results” appear, check image retrieval before treating it as an index
miss; Wikimedia hotlinks failed in the recorded tests.
