...
impl BrowserContextHandle {
    ...
    #[cfg(any(test, feature = "test-support"))]
    #[doc(hidden)]
    pub fn suspend_navigation_resource_owner_for_test(
        &self,
        handle: WebContentsHandle,
    ) -> Result<impl std::future::Future<Output = ()> + use<>, String> {
        let (restore, restored) = self.browser.execute(move |browser| {
            let mut retired_owner = crate::runtime::RendererBrowserContextRuntime::new();
            let retired_engine =
                crate::runtime::NavigationEngine::new_with_fetch_config_and_browser_context_access(
                    Default::default(),
            ...
            let original = std::mem::replace(
                browser
                    ...
                    .navigation_engine_for_test_mut(handle)
                    .ok_or("navigation engine unavailable")?,
                retired_engine,
            );
            let (restore, resume) = oneshot::channel();
            let (completed, restored) = oneshot::channel();
            let sender = browser.local_sender.clone();
            tokio::task::spawn_local(async move {
                let _ = resume.await;
                let _ = sender.send(Box::new(move |browser| {
                    if let Ok(context) = browser.context_mut(handle.context())
                        && let Some(engine) = context.navigation_engine_for_test_mut(handle)
                    {
                        *engine = original;
                    }
    ...
    forward_context_update! {
        ...
        fn set_default_extra_headers(headers: moli_fetch::RequestHeaders) -> ();
