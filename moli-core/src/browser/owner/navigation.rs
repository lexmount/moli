...
impl NavigationWorkRegistry {
    ...
    fn load_mut(&mut self, id: NavigationWorkId) -> Result<&mut PhysicalNavigationLoad, String> {
        let entry = self.work.get_mut(&id).ok_or_else(unavailable)?;
        match &mut entry.value {
            NavigationWork::Load(load) => Ok(load),
            _ => Err(unavailable()),
        }
    }
...
impl BrowserNavigationLoad {
    ...
    pub fn set_redirect_state(
        ...
        let id = self.work()?;
        self.context.browser.execute(move |browser| {
            browser
                .navigation_work
                .load_mut(id)?
                .set_redirect_state(headers, chain);
            Ok(())
        })?
    }
...
impl BrowserInterceptedNavigationLoad {
    ...
    pub async fn fetch_streaming(
        mut self,
        auth: Option<SubresourceAuthCredentials>,
    ) -> anyhow::Result<BrowserInterceptedNavigationResponse<StreamingRawResponse>> {
        let response = self
            .load
    ...
    pub async fn fetch_auth(
        mut self,
        auth: SubresourceAuthCredentials,
    ) -> anyhow::Result<BrowserInterceptedNavigationResponse<RawResponse>> {
        let response = self
            .load
...
impl BrowserInterceptedNavigationResponse<RawResponse> {
    pub fn retry(self) -> Result<BrowserInterceptedNavigationLoad, String> {
        let (mut work, response) = self.into_parts();
        let (response, observations) = response.into_parts_with_observation_journal();
        let mut request = moli_fetch::Request::get_with_url(work.requested_url);
        request.method = work.method;
        request.body = work.body;
        request.request_headers = work.headers;
        let id = work.load.work()?;
        let request = work.load.context.browser.execute(move |browser| {
            browser
                .navigation_work
                .load_mut(id)
                .map(|load| load.redirected_request(response, request))
        })??;
        work.requested_url = request.url;
