...
impl Browser {
    ...
    pub(super) fn activate_web_contents(
        &mut self,
        selected: WebContentsHandle,
    ) -> Result<PendingWebContentsActivation, String> {
        let browser = self;
        let context = browser.context_mut(selected.context())?;
        let selected_has_dialog = context.web_contents_has_pending_javascript_dialog(selected)?;
...
impl BrowserHandle {
    pub fn activate_web_contents(
        ...
        self.execute(move |browser| browser.activate_web_contents(selected))?
    }
}
