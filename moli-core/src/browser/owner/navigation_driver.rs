...
impl Browser {
    ...
    fn native_navigation_policy(
        ...
        Ok(context.inherited_document_policy(
            config,
...
async fn navigate(
    ...
    let synthetic = match decision {
        NavigationDecision::Request {
            ...
            body,
            headers,
            redirect_headers: replacement_redirect_headers,
        } => {
            redirect_headers = replacement_redirect_headers;
            requested_url = url;
            method = replacement;
    ...
    .await?;
    load.set_redirect_state(redirect_headers, Vec::new());
                ...
                if matches!(head.status, 401 | 407) {
                    ...
                    match decision {
                        NavigationDecision::Authenticate {
                            ...
                            retry.request_headers = request_headers;
                            let retry = load.redirected_request(head, retry);
                            requested_url = retry.url;
                            method = retry.method;
                            request_body = retry.body;
                            request_headers = retry.request_headers;
...
async fn commit_response_document(
    ...
    let prepared = preparation.await.map_err(|error| format!("{error:#}"))?;
...
fn navigation_fetch_failure(
    failure: anyhow::Error,
    requested_url: Url,
) -> anyhow::Result<(
    crate::browser::NavigationFetchFailure,
    moli_fetch::NetworkObservationJournal,
)> {
    let (error, request, observations) = if let Some(network) =
        failure.downcast_ref::<moli_fetch::NetworkFetchFailureContext>()
    {
        (
            crate::browser::NavigationError {
                unreachable_url: network
                    .request_context()
                    .map(|request| request.current_url().clone())
                    .unwrap_or(requested_url),
                error_text: network.network_error_text().to_owned(),
            },
            network.request_context().cloned(),
            network.observation_journal().clone(),
        )
    } else if let Some(offline) = failure.downcast_ref::<crate::browser::NavigationNetworkError>() {
        (
            crate::browser::NavigationError {
                unreachable_url: offline.unreachable_url.clone(),
                error_text: offline.kind.error_text().to_owned(),
            },
            Some(moli_fetch::NetworkFetchFailureRequestContext::new(
                offline.unreachable_url.clone(),
                offline.request_method.clone(),
                None,
                offline.request_headers.clone(),
                Vec::new(),
            )),
            Default::default(),
        )
...
async fn commit_fetch_error(
    ...
    let (failure, observations) =
        navigation_fetch_failure(failure, requested_url).map_err(|error| format!("{error:#}"))?;
    ...
    let captured = match source {
        ...
        DocumentBodySource::BufferedRaw { response, .. } => {
            let bytes = response
                ...
                .try_into_materialized_bytes()
                .map_err(|_| {
                    anyhow::anyhow!("buffered native response has no materialized bytes")
                })?;
...
) -> anyhow::Result<(ExternalRawDocumentBodyStream, NativeBodyCapture)> {
    let mut reader = captured.chunk_reader(64 * 1024)?;
    let (completion, completed) = oneshot::channel();
    let (sender, body) = ExternalRawDocumentBodyStream::channel(completed);
...
#[derive(Clone, Debug)]
struct SharedBodyCaptureError(std::sync::Arc<anyhow::Error>);
...
fn complete_body_capture(
    ...
    match result {
        ...
        Err(error) => {
            let error = SharedBodyCaptureError(std::sync::Arc::new(error));
