...
impl PausedDocumentTransfer {
    ...
    pub async fn finish_body_stream_async(
        self,
    ) -> Result<(NavigationRequestLoadPolicy, DocumentBodySource), (Self, anyhow::Error)> {
        let Self {
            request_load_policy,
            state,
            decision_claim,
...
impl ActiveDocumentBodyStreamState {
    ...
    async fn read_async(&mut self, size: Option<usize>) -> Result<(Vec<u8>, bool), anyhow::Error> {
    ...
    async fn read_next_chunk_async(&mut self) -> anyhow::Result<()> {
        if self.finished {
            return Ok(());
        }
