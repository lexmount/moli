...
pub struct AdmittedDocumentMaterialization {
    identity: DocumentNavigationIdentity,
...
impl PreparedDocumentNavigation {
    ...
    async fn apply_document_policy(
        mut self,
        interception: (bool, Option<crate::page::SubresourceResourceType>),
        permissions: &[crate::page::PermissionOverrideRegistration],
    ) -> anyhow::Result<Self> {
        use anyhow::Context;
...
pub struct CommittedDocumentNavigation {
    pub web_contents: WebContentsId,
    pub frame_slot: MainFrameSlotId,
    ...
    pub retirement: RetiringDocument,
    pub post_response_continuation: Option<RendererPageCommandPostResponseContinuation>,
}

/// Retirement has no commit authority and never retains a Browser registry borrow.
pub struct RetiringDocument(Option<Page>);
...
pub struct CommittedDocumentLifecycle {
    ...
    pub artifacts: RendererPageCreationArtifacts,
}

impl RetiringDocument {
    pub(crate) fn from_page(page: Option<Page>) -> Self {
        Self(page)
    }
...
impl WebContents {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn start_loaded_document_navigation(
        &self,
        navigation: NavigationId,
        page: Page,
        destination: DocumentNavigationDestination,
...
mod tests {
    ...
    async fn loaded_candidate_admission_clears_revoked_permissions() {
        ...
        let original = page
            ...
            .await
            .unwrap();
        page.set_permission_overrides_async(&[crate::page::PermissionOverrideRegistration {
    ...
    async fn failed_candidate_policy_never_retires_the_committed_document() {
        ...
        for (interception, permissions) in [
            ...
            (
                (false, None),
                vec![crate::page::PermissionOverrideRegistration {
                    permission: serde_json::json!({"name": "geolocation"}),
                    setting: "granted".into(),
                    origin: None,
                    embedded_origin: None,
