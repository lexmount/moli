...
pub struct OriginStorageUsage {
    pub local_storage_usage: u64,
    pub indexed_db_usage: u64,
    pub storage_buckets_usage: u64,
    pub total_usage: u64,
}

#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct SiteDataClearOptions {
    pub cookies: bool,
...
pub struct StoragePartition {
    pub kind: StoragePartitionKind,
    pub handles: BrowserContextStoragePartitionHandles,
...
impl StoragePartition {
    ...
    pub fn usage_for_origin(&self, serialized_origin: &str) -> Result<OriginStorageUsage, String> {
        ...
        let indexed_db_usage = crate::storage::indexed_db_origins_with_prefix_usage_bytes(
    ...
    fn storage_bucket_usage_for_origin(&self, serialized_origin: &str) -> Result<u64, String> {
        ...
        for identity in bucket_identities {
            let indexed_db_usage = crate::storage::indexed_db_origin_usage_bytes(
                self.indexed_db_manager(),
    ...
    fn complete_storage_bucket_deletions(
        ...
        for cleanup in cleanups {
            crate::storage::complete_storage_bucket_deletion(bucket_store, &cleanup)
    ...
    pub fn clear_site_data_for_origin(
        ...
        if options.indexed_db {
            crate::storage::clear_indexed_db_origins_with_prefix(
                self.indexed_db_manager(),
                &moli_storage_key::storage_key_prefix_for_origin(&serialized_origin),
            )?;
        }
    ...
    pub fn clear_site_data_for_storage_key(
        ...
        if options.indexed_db {
            crate::storage::clear_indexed_db_origin(
                self.indexed_db_manager(),
    ...
    pub fn replace_storage_bucket_store(&mut self, storage_bucket_store: SharedStorageBucketStore) {
    ...
    pub fn storage_quota_for_origin(&self, origin: &str) -> (f64, bool) {
        self.storage_quota_overrides
            ...
            .unwrap_or((
                crate::storage::DEFAULT_ORIGIN_STORAGE_QUOTA_BYTES as f64,
                false,
            ))
    }
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn http_cache_configuration(&self) -> (Option<&std::path::Path>, Option<u64>) {
