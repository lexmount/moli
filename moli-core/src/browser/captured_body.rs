...
pub const DEFAULT_BODY_MATERIALIZE_LIMIT: usize = 64 * 1024 * 1024;
...
enum CapturedBodyInner {
    ...
    Subresource(crate::page::SubresourceResponseBody),
}
...
impl CapturedBody {
    ...
    pub fn from_shared_bytes(body: Arc<[u8]>) -> Self {
        Self {
            inner: Arc::new(CapturedBodyInner::SharedMemory(body)),
        }
    }

    pub fn from_bytes_spooled(body: Vec<u8>) -> Self {
    ...
    pub fn from_optional_renderer_synthetic_response_body(
        body: Option<crate::page::RendererSyntheticResponseBody>,
    ) -> Self {
        Self::from_bytes(body.map(|body| body.into_body_bytes()).unwrap_or_default())
    }

    pub fn from_subresource_response_body(body: &crate::page::SubresourceResponseBody) -> Self {
        Self {
    ...
    pub fn materialize_bytes_limited(&self, limit: usize) -> Result<Vec<u8>> {
    ...
    pub fn materialize_lossy_string(&self) -> Result<String> {
        self.materialize_bytes()
            .map(|bytes| String::from_utf8_lossy(&bytes).into_owned())
    }

    pub fn chunk_reader(&self, chunk_size: usize) -> Result<CapturedBodyChunkReader> {
        if chunk_size == 0 {
            return Err(anyhow!(
...
pub struct CapturedBodyChunkReader {
...
pub fn ensure_materialize_limit(len: usize, limit: usize) -> Result<()> {
    if len > limit {
        Err(anyhow!(
...
impl CapturedBodyWriter {
    ...
    pub fn finish(mut self) -> Result<CapturedBody> {
        self.finish_in_place()
    }

    pub fn finish_in_place(&mut self) -> Result<CapturedBody> {
        if let Some(file) = self.file.as_mut() {
            file.flush()
...
mod tests {
    ...
    fn captured_body_from_subresource_response_body_reads_shared_source() -> Result<()> {
        let subresource_body =
            crate::page::SubresourceResponseBody::from_bytes(b"hello world".to_vec());
