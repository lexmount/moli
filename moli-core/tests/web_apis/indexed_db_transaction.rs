...
async fn indexed_db_database_names_preserve_utf16_identity_across_realms_and_queues() -> Result<()>
{
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;
    let fixture = include_str!("fixtures/indexeddb-database-names.js");
    for target in ["window", "child", "worker"] {
        let source = format!(
            "{fixture}\ndatabaseNamesProbe('database-names-{target}').then(finish, error => finish({{state: 'error', error: String(error), checks: databaseNameChecks}}));"
        );
...
async fn indexed_db_clear_validates_before_requests_and_preserves_callee_errors() -> Result<()> {
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;
    let fixture = include_str!("fixtures/indexeddb-clear.js");
    for target in ["window", "child", "worker"] {
        let source = format!(
            "{fixture}\nclearProbe('clear-{target}').then(finish, error => finish({{state: 'error', error: String(error), checks: clearChecks}}));"
        );
        let result = run_probe(&browser, &server, target, &source).await?;
        assert_eq!(result["state"], "pass", "{target}: {result}");
        assert_eq!(
            result["checks"].as_array().unwrap().len(),
            if target == "worker" { 94 } else { 282 },
            "{target}: {result}"
        );
    }
    server.shutdown().await;
...
#[tokio::test(flavor = "multi_thread")]
async fn indexed_db_schema_attributes_preserve_native_state_and_handle_identity() -> Result<()> {
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;
    let fixture = include_str!("fixtures/indexeddb-schema-attributes.js");
    for target in ["window", "child", "worker"] {
        let source = format!(
            "{fixture}\nschemaAttributesProbe('schema-attributes-{target}').then(finish, error => finish({{state: 'error', error: String(error), checks: schemaAttributeChecks}}));"
        );
        let result = run_probe(&browser, &server, target, &source).await?;
        assert_eq!(result["state"], "pass", "{target}: {result}");
        assert!(
            result["checks"].as_array().unwrap().len() > 100,
            "{target}: {result}"
        );
    }
    server.shutdown().await;
...
#[tokio::test(flavor = "multi_thread")]
async fn indexed_db_store_names_follow_upgrade_commit_abort_and_utf16_identity() -> Result<()> {
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;
    let fixture = include_str!("fixtures/indexeddb-store-rename.js");
    for target in ["window", "child", "worker"] {
        let source = format!(
            "{fixture}\nstoreRenameProbe('store-rename-{target}').then(finish, error => finish({{state: 'error', error: String(error), checks: storeRenameChecks}}));"
        );
        let result = run_probe(&browser, &server, target, &source).await?;
        assert_eq!(result["state"], "pass", "{target}: {result}");
        assert!(
...
#[tokio::test(flavor = "multi_thread")]
async fn indexed_db_index_names_follow_upgrade_commit_abort_and_utf16_identity() -> Result<()> {
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;
    let fixture = include_str!("fixtures/indexeddb-index-rename.js");
    for target in ["window", "child", "worker"] {
        let source = format!(
            "{fixture}\nindexRenameProbe('index-rename-{target}').then(finish, error => finish({{state: 'error', error: String(error), checks: renameChecks}}));"
        );
        let result = run_probe(&browser, &server, target, &source).await?;
        assert_eq!(result["state"], "pass", "{target}: {result}");
        assert!(
