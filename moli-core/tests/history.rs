...
#[tokio::test]
async fn history_same_turn_back_then_forward_preserves_both_popstates() -> Result<()> {
    let server = FixtureServer::spawn().await?;
    let browser = Browser::new(AppConfig::default())?;

    let mut page = browser
        .fetch(&server.url("/compat/history-back-forward-same-turn-traverses-in-order"))
        .await?;
