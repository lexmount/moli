    ...
    fn place_floated_box_inner(
        ...
        let end_idx = match end {
            None => {
                ...
                // The new float outlives the existing exclusion segments. Its
                // remaining height must retain an exclusion of its own after
                // the shorter floats end.
                if end_y > last_y_end {
                    self.segments.push(Segment { y: last_y_end..end_y, insets: [0.0, 0.0], has_float: [false; 2] });
                }
                self.segments.len() - 1
            }
            Some(mut end_idx) => {
...
mod tests {
    ...
    #[test]
    fn taller_float_retains_exclusion_after_the_existing_segments_end() {
        for short_direction in [FloatDirection::Left, FloatDirection::Right] {
            let tall_direction = match short_direction {
                FloatDirection::Left => FloatDirection::Right,
                FloatDirection::Right => FloatDirection::Left,
            };
            let mut context = FloatContext::new();
            context.set_width(57.0);
            context.place_floated_box(Size { width: 16.0, height: 16.0 }, 0.0, [0.0; 2], short_direction, Clear::None);
            context.place_floated_box(Size { width: 16.0, height: 40.0 }, 0.0, [0.0; 2], tall_direction, Clear::None);
            let first = context.find_content_slot(0.0, [0.0; 2], Clear::None, None);
            assert_eq!(first.width, 25.0);
            let second = context.find_content_slot(0.0, [0.0; 2], Clear::None, first.segment_id);
            assert_eq!((second.y, second.width), (16.0, 41.0));
            assert_eq!(context.find_content_slot(20.0, [0.0; 2], Clear::None, None).width, 41.0);
            assert!(context.has_active_floats(20.0));
            assert!(!context.has_active_floats(40.0));
            let next = context.place_floated_box(
                Size { width: 40.0, height: 4.0 },
                0.0,
                [0.0; 2],
                short_direction,
                Clear::None,
            );
            assert_eq!(next.y, 16.0, "a later float must use the surviving exclusion band");
        }
    }
}
