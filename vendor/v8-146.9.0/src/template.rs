...
pub struct NativeDataPropertyConfiguration<'s> {
...
impl<'s> NativeDataPropertyConfiguration<'s> {
  pub fn new(getter: impl MapFnTo<AccessorNameGetterCallback>) -> Self {
    Self {
      getter: getter.map_fn_to(),
      setter: None,
...
impl Template {
  ...
  ///
  /// V8 replaces the lazy property with an ordinary data property after the
  /// callback returns. Assigning to it before the first read replaces it
...
impl ObjectTemplate {
  ...
  /// Mark object instances created from this template as undetectable.
  #[inline(always)]
  pub fn mark_as_undetectable(&self) {
    unsafe { v8__ObjectTemplate__MarkAsUndetectable(self) };
  }

  /// Marks objects created from this template as code-like so V8 passes them
  /// through the host string-code-generation callback for `eval` and function
  /// constructors.
  #[inline(always)]
  ...
  /// Make object instances created from this template callable.
  #[inline(always)]
  pub fn set_call_as_function_handler(
    &self,
    callback: impl MapFnTo<FunctionCallback>,
  ) {
    self.set_call_as_function_handler_with_data(callback, None);
  }
  ...
  pub fn set_call_as_function_handler_with_data(
    &self,
    callback: impl MapFnTo<FunctionCallback>,
    data_or_null: Option<Local<Value>>,
  ) {
    self.set_call_as_function_handler_raw(callback.map_fn_to(), data_or_null);
  }

  /// Make object instances created from this template callable.
  #[inline(always)]
  pub fn set_call_as_function_handler_raw(
    &self,
    callback: FunctionCallback,
    data_or_null: Option<Local<Value>>,
  ) {
    unsafe {
      v8__ObjectTemplate__SetCallAsFunctionHandler(
        self,
        callback,
        data_or_null.map_or_else(null, |value| &*value),
      );
    }
  }
  ...
  pub fn set_native_data_property(
    ...
    self
      .set_native_data_property_with_configuration(key, NativeDataPropertyConfiguration::new(getter));
  }
  ...
  /// Enables embedder-defined access checks and dispatches denied cross-context
  /// accesses through the supplied named and indexed handlers.
