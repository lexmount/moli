...
pub struct UnwrappedObject<'s> {
  ...
  pub object_group: UniquePtr<StringBuffer>,
}
...
impl V8InspectorSession {
  ...
  pub fn state(&self) -> Vec<u8> {
    unsafe {
      ...
      v8_inspector__V8InspectorSession__State__copy(
        state,
        bytes.as_mut_ptr(),
      );
  ...
  unsafe fn unwrap_object_unchecked<'s>(
    ...
    if success {
      Ok(UnwrappedObject {
        value: unsafe { Local::from_raw(value) }
          .expect("V8InspectorSession::unwrapObject returned a null value"),
        context: unsafe { Local::from_raw(context) }
          .expect("V8InspectorSession::unwrapObject returned a null context"),
        object_group: unsafe { UniquePtr::from_raw(object_group) },
      })
    } else {
      Err(unsafe { UniquePtr::from_raw(error) })
    }
  }
}
...
impl V8Inspector {
  ...
  pub fn reset_context_group(&self, context_group_id: i32) {
    unsafe {
      v8_inspector__V8Inspector__resetContextGroup(
        self.raw(),
        context_group_id,
      )
    }
