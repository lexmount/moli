...
///   - scoped: microtask invocation is controlled by MicrotasksScope objects;
///   - auto: microtasks are invoked when the script call depth decrements to
///     zero.
...
pub type ModifyCodeGenerationFromStringsCallback = unsafe extern "C" fn(
  Local<Context>,
  Local<Value>,
  bool,
  *mut *const String,
...
unsafe extern "C" {
  ...
  fn v8__Isolate__SetModifyCodeGenerationFromStringsCallback(
    isolate: *mut RealIsolate,
    callback: Option<ModifyCodeGenerationFromStringsCallback>,
  );
...
impl Isolate {
  ...
  // Must match kModifyCodeGenerationCallbackSlot in code_generation_ext.cc.
  const MODIFY_CODE_GENERATION_CALLBACK_SLOT: u32 = 1;
  ...
  pub(crate) fn initialize(&mut self, create_param_allocations: Box<dyn Any>) {
    ...
    assert!(
      self
        .get_data_internal(Self::MODIFY_CODE_GENERATION_CALLBACK_SLOT)
        .is_null()
    );
  ...
  /// Prepare annex teardown while keeping `ANNEX_SLOT` pointing at live
  /// storage. V8's final teardown can run weak callbacks, and those callbacks
  /// resolve the finalizer map through the isolate annex.
  ///
  /// # Safety
  ///
  /// Called exactly once for an isolate teardown path.
  unsafe fn prepare_annex_for_dispose(
    ...
    let create_param_allocations = unsafe {
      std::mem::replace(
        &mut (*annex_ptr).create_param_allocations,
        Box::new(()),
      )
    };
    // Drop user-owned slots outside a borrow of the annex. Their destructors
    // may re-enter isolate state while the annex is still authoritative.
  ...
  /// Run guaranteed finalizers without retaining a borrow of the annex map.
  /// A finalizer may drop its own `Weak`, which removes its id from the now
  /// empty authoritative map.
  ///
  /// # Safety
  ///
  /// `annex_ptr` must point to the live annex stored in `ANNEX_SLOT`.
  unsafe fn run_remaining_guaranteed_finalizers(
    annex_ptr: *mut IsolateAnnex,
  ) {
    let mut finalizers =
      unsafe { std::mem::take(&mut (*annex_ptr).finalizer_map) };
    for finalizer in finalizers.drain() {
  ...
  /// Mark thread-safe handles disposed only after pre-teardown finalizers have
  /// reset their V8 globals. The old `Weak` implementation consults this
  /// pointer before resetting a global node.
  ///
  /// # Safety
  ///
  /// `annex_ptr` must point to this isolate's live annex.
  unsafe fn mark_annex_isolate_disposed(annex_ptr: *mut IsolateAnnex) {
    let annex = unsafe { &mut *annex_ptr };
    let _lock = annex.isolate_mutex.lock().unwrap();
    annex.isolate = null_mut();
  }

  /// Release the isolate-owned Arc reference after V8 can no longer invoke
  /// weak callbacks. Cloned `IsolateHandle`s may keep the disposed annex alive.
  ///
  /// # Safety
  ///
  /// V8 must already have fully disposed the isolate.
  unsafe fn finish_annex_dispose(annex_ptr: *mut IsolateAnnex) {
    unsafe { Self::run_remaining_guaranteed_finalizers(annex_ptr) };
    unsafe { drop(Arc::from_raw(annex_ptr)) };
  }

  /// Snapshot teardown does not run `OwnedIsolate::drop`, so it completes the
  /// annex cleanup synchronously and clears the internal annex slot.
  unsafe fn dispose_annex(&mut self) -> Box<dyn Any> {
    ...
    self.set_data_internal(Self::ANNEX_SLOT, null_mut());
    unsafe { drop(Arc::from_raw(annex_ptr)) };
  ...
  /// Sets the callback invoked when an object access check fails and no
...
impl Drop for OwnedIsolate {
  fn drop(&mut self) {
    unsafe {
      ...
      // Context annex finalizers own weak V8 globals. Run them while the old
      // IsolateHandle still exposes the live isolate so `Weak::drop` resets
      // those nodes instead of freeing callback storage underneath V8.
      ...
      // Keep the annex in `ANNEX_SLOT` until final teardown GC has finished;
      // weak second-pass callbacks still access its finalizer map.
      self.dispose();
