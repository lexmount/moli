    ...
    for (status, headers, commits_child) in [
        ...
        for recover in [false, true] {
            ...
            let navigation = context
                ...
                .unwrap();
            let BrowserNavigationOutcome::Document(commit) = navigation.wait().await.unwrap()
            else {
            ...
            }).await.expect("a completed child response retains its native receipt without DevTools");
            assert_eq!(
                load.is_some(),
                commits_child,
            ...
            let peer_navigation = context
                ...
                .unwrap();
            let BrowserNavigationOutcome::Document(peer_commit) =
            ...
            assert_eq!(snapshot.network_requests.iter().filter(|request| request.document == commit.document && matches!(&request.state, moli_core::browser::NetworkRequestState::ChildDocument(response) if response.snapshot.request_url == child_url && response.snapshot.status == status)).count(), 1);
            ...
            assert!(
                !events
                    .iter()
                    .any(|event| event.protocol_method() == Some("Network.loadingFailed")),
