...
async fn pending_emulation_completion_follows_the_exact_target_across_activation_and_navigation() {
    ...
    let pending = ctx
        .conn
        .start_rebuild_resource_runtime_for_session_owner(Some("SID-1"))
        .unwrap()
        .unwrap();
    let resource_completion = pending.wait().await;
...
async fn multi_session_emulation_separates_handler_input_from_target_effective_state() {
    ...
    assert_eq!(
        {
            let c = ctx.conn.browser_context.as_ref().unwrap();
            c.target_emulation_policy(c.active_target_id().unwrap())
                .unwrap()
        }
        .max_touch_points,
        0
