//! Legacy staged navigation fixtures. No production Fetch or navigation command
//! calls these loaders; migrate their white-box callers to native Browser events
//! before removing the remaining test-only staged construction capabilities.
...
use moli_fetch::{
    BrowserNavigationRequestKind, NetworkFetchFailureContext, NetworkFetchResult,
...
impl ResponseCommitReady {
    async fn finish_materialization<P>(
        ...
        let document_progress_transfer = CompletedDocumentProgressTransfer::new_captured(
            captured_body,
            self.synthetic_body,
            body_network_progress_state,
        );
...
pub(crate) struct NavigationFixtureLoad {
...
impl NavigationFixtureLoad {
    pub(crate) async fn run(self) -> Result<NavigationLoadOutcome, String> {
...
impl CdpConnection {
    ...
    pub(crate) async fn load_navigation_via_runtime_async(
        &mut self,
        raw_url: &str,
    ) -> Result<LoadedNavigation<PreparedDocumentNavigation>, String> {
    ...
    pub(crate) async fn prepare_navigation_fixture_for_owner_and_token_async(
        ...
        let request_headers = load_inputs.extra_http_headers.clone();
        let load = self.navigation_load_fixture_for_token(owner, &load_inputs, token)?;
        let navigation = NavigationFixtureLoad {
            load,
            reply_boundary: RendererReplyBoundary::Stage,
        ...
        match navigation {
            ...
            NavigationLoadOutcome::NetworkFailure(error) => Err(error),
        }
    }
    ...
    pub(crate) async fn load_page_via_runtime_async(
        &mut self,
        raw_url: &str,
    ) -> Result<PreparedDocumentNavigation, String> {
