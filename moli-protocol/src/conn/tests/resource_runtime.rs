        ...
        native
            .resolve_navigation_decision(
                ...
                NavigationDecision::Fulfill {
                    ...
                    body: b"<script>document.title = navigator.maxTouchPoints</script>".to_vec(),
                }
            )
            .unwrap()
    );
...
fn initial_storage_is_reused_without_an_unowned_resource_runtime() {
    ...
    for inputs in [&first_inputs, &second_inputs] {
        assert!(matches!(
            ...
            Err(message) if message.to_string() == "resource request fixture requires an installed WebContents"
        ));
    }
    assert_eq!(
        conn.moli_memory_diagnostics()["isolateScope"]["estimatedRendererOwnerCount"],
...
async fn start_cookie_navigation_pause(
    ...
    context.store_response_cookie_headers_for_test(
        requested_url,
        &[("set-cookie".into(), b"sid=1; Path=/document".to_vec())],
    );
