...
impl BrowserContext {
    pub(in crate::conn) fn inherited_document_policy_for_target(
        &self,
        target_id: &str,
        fetch_defaults: moli_fetch::FetchConfig,
        browser_globals: &crate::conn::BrowserGlobalOverrides,
    ) -> moli_core::browser::web_contents::InheritedDocumentPolicy {
        let mut policy = self.browser_context.inherited_document_policy(
            fetch_defaults,
        ...
        if let Some(target) = self.page_target(target_id) {
            policy.navigator_queries = target.devtools_sessions.navigator_emulation.effective();
        }
        policy
    }
    ...
    pub(in crate::conn) fn start_initial_document_for_target(
        ...
        let inherited =
            self.inherited_document_policy_for_target(target_id, fetch_defaults, browser_globals);
        let handle = self
...
impl BrowserContext {
    ...
    pub(crate) fn project_document_commit_snapshot(
        ...
        if self
            .projected_renderer_document_lifecycle_binding_for_target(target_id)
