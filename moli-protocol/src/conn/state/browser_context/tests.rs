...
fn context_header_defaults_outlive_projection_without_replacing_network_policy() {
    ...
    let physical = {
        ...
        projection.set_default_extra_headers(Default::default());
...
fn context_browser_identity_preserves_independent_inputs_and_update_time_fallback() {
    ...
    for locale_first in [false, true] {
        ...
        if locale_first {
            context
                .set_default_locale_override(Some("fr-FR".into()), &fallback)
                .unwrap();
            assert_eq!(
        ...
        context
            .set_default_locale_override(None, &next_fallback)
            .unwrap();
        assert!(context.default_browser_identity_override().is_none());
...
fn context_browser_identity_outlives_projection() {
    ...
    let physical = {
        ...
        projection
            .set_default_locale_override(Some("fr-FR".into()), &Default::default())
            .unwrap();
        assert_eq!(
            projection
                .effective_active_browser_identity_override_owned()
...
fn context_emulation_defaults_outlive_projection_without_inherited_mirrors() {
    ...
    let (physical, snapshot) = {
        ...
        projection
            ...
            .unwrap();
        projection
            .set_default_timezone_override(Some("Asia/Tokyo".into()))
            .unwrap();
