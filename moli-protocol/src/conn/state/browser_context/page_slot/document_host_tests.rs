...
async fn document_policy_completion_rejects_replacement_document() {
    ...
    assert_eq!(
        owner.finish_document_policy_batch(surface_completed),
        Err("Document changed".to_owned())
    );
    assert_eq!(owner.document_handle_for_target(TARGET), Some(replacement));

    retired.close_async().await.unwrap();
    let mut replacement_page = owner.clear_target_page_for_test(TARGET).unwrap();
    let observed = replacement_page
    ...
    replacement_page.close_async().await.unwrap();
}
