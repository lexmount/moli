...
#[derive(Debug)]
pub(crate) struct NavigationRequestInterception {
    requested_url: Url,
    method: String,
    body: Option<Vec<u8>>,
...
#[derive(Debug)]
pub(crate) struct ClaimedNavigationRequest {
...
impl ClaimedNavigationRequest {
    ...
    pub(crate) fn permit(&self) -> NavigationInterceptionPermit {
        self.permit
    }
...
pub(crate) struct InterceptedNavigationLoad {
    ...
    body: Option<Vec<u8>>,
    pub(in crate::conn) headers: moli_fetch::RequestHeaders,
...
impl InterceptedNavigationLoad {
    ...
    ) -> anyhow::Result<InterceptedNavigationResponse<StreamingRawResponse>> {
        let response = self
            .load
            ...
            .await
            .with_context(|| format!("failed to fetch page `{}`", self.requested_url))?;
    ...
    ) -> anyhow::Result<InterceptedNavigationResponse<RawResponse>> {
        let response = self
            .load
    ...
    pub(in crate::conn) fn with_response<R>(
        mut self,
    ...
    pub(in crate::conn) fn into_request_parts(
        self,
    ) -> (
...
impl InterceptedNavigationResponse<RawResponse> {
    pub(crate) fn retry(self) -> InterceptedNavigationLoad {
        ...
        let (response, observations) = response.into_parts_with_observation_journal();
        let redirect_headers = if response.redirect_chain.len() > work.load.redirect_chain.len() {
            work.load.redirect_headers.take()
        } else {
            None
        };
        let mut request = moli_fetch::Request::get_with_url(work.requested_url.clone())
            .with_redirect_headers(redirect_headers);
        request.method = work.method;
        request.body = work.body;
        request.request_headers = work.headers;
        for redirect in response
            .redirect_chain
            .iter()
            .skip(work.load.redirect_chain.len())
        {
            request.apply_redirect_status(redirect.status);
        }
        work.requested_url = response.final_url;
        work.method = request.method;
        work.body = request.body;
        work.headers = request.request_headers;
        work.load.redirect_chain = response.redirect_chain;
...
pub(super) struct PausedNavigationResponse {
    pub(super) request: BrowserRequestId,
    pub(super) transfer: crate::conn::PausedDocumentTransfer,
}
...
impl WebContents {
    pub(in crate::conn::state) fn pause_navigation_request(
        &mut self,
    ...
    pub(in crate::conn::state) fn take_navigation_request(
        &mut self,
        permit: NavigationInterceptionPermit,
    ...
    pub(in crate::conn::state) fn start_claimed_navigation_request(
        &mut self,
        ...
        let mut load = self.start_navigation_load(permit.navigation, request.policy, inherited)?;
        load.redirect_headers = request.redirect_headers;
    ...
    pub(in crate::conn::state) fn start_navigation_load_for_interception(
        &mut self,
        permit: NavigationInterceptionPermit,
        policy: NavigationRequestLoadPolicy,
        inherited: InheritedDocumentPolicy,
    ...
    pub(in crate::conn::state) fn pause_navigation_response(
        &mut self,
        navigation: NavigationId,
    ...
    pub(in crate::conn::state) fn take_navigation_response(
        &mut self,
        permit: NavigationInterceptionPermit,
    ...
    pub(in crate::conn::state) fn restore_navigation_response(
        &mut self,
        permit: NavigationInterceptionPermit,
        transfer: crate::conn::PausedDocumentTransfer,
...
mod tests {
    ...
    fn paused_response(url: Url) -> crate::conn::PausedDocumentTransfer {
        let response = RawResponse::from_head_and_body(
            ResponseHead {
                final_url: url.clone(),
                status: 200,
        ...
        crate::conn::PausedDocumentTransfer::pending(
            NavigationRequestLoadPolicy::DocumentInitiated,
            crate::conn::DocumentBodySource::BufferedRaw {
                requested_url: url,
