...
impl ClaimedFetchNavigation {
    ...
    pub(crate) fn apply_overrides(
        ...
        if let Some(headers) = headers {
            self.pending.navigation.redirect_headers = match &headers {
                moli_fetch::RequestHeaderOverride::CurrentRequest {
                    redirect_headers, ..
                } => Some(
                    redirect_headers
                        .clone()
                        .unwrap_or_else(|| self.pending.navigation.request_headers.clone()),
                ),
                moli_fetch::RequestHeaderOverride::RedirectChain(_) => None,
            };
            self.pending.navigation.request_headers = headers.headers().clone();
        }
    }
...
impl PausedDocumentTransfer {
    ...
    ) -> Result<anyhow::Result<NavigationLoadOutcome>, Self> {
        let Self {
            request_load_policy,
            state,
        } = self;
