...
impl ParkableImage {
    ...
    pub(crate) fn park(&self) -> io::Result<ParkOutcome> {
        self.park_impl(false)
    }

    /// Memory-pressure path: bypasses the time deadline, but never a live
    /// reader or another storage transition.
    pub(crate) fn force_park(&self) -> io::Result<ParkOutcome> {
        self.park_impl(true)
    }

    fn park_impl(&self, ignore_deadline: bool) -> io::Result<ParkOutcome> {
