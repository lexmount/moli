...
fn unused_image_observes_delay_but_used_image_can_park_immediately() {
    ...
    assert!(matches!(image.park().unwrap(), ParkOutcome::Delayed { .. }));

    let snapshot = image.snapshot().unwrap();
    assert_eq!(image.park().unwrap(), ParkOutcome::InUse);
    drop(snapshot);
    assert_eq!(image.park().unwrap(), ParkOutcome::Parked);
}

#[test]
fn park_images_respects_delay_while_force_park_images_bypasses_it() {
    let (_, manager) = manager(ParkableImagePolicy {
    ...
    assert_eq!(image.snapshot().unwrap().as_ref(), &[7; 4096]);
}
...
fn forced_sweep_bypasses_deadlines_but_not_live_readers() {
    ...
    let report = manager.force_park_images_with_report();
...
fn one_extent_round_trips_and_reparks_without_another_write() {
    ...
    assert_eq!(image.park().unwrap(), ParkOutcome::Parked);
    assert_eq!(image.park().unwrap(), ParkOutcome::AlreadyParked);
    assert_eq!(
        image.diagnostics().storage,
        ParkableImageStorageState::Parked
    );
...
fn below_minimum_and_capacity_failure_keep_memory() {
    ...
    assert_eq!(small.park().unwrap(), ParkOutcome::BelowMinimum);

    let large = manager.from_frozen_bytes(vec![2; 8]);
    assert_eq!(large.park().unwrap(), ParkOutcome::Unavailable);
    assert_eq!(
...
fn failed_write_keeps_the_image_resident_and_disables_the_pool() {
    ...
    assert_eq!(image.snapshot().unwrap().as_ref(), bytes);
    assert_eq!(image.park().unwrap(), ParkOutcome::Unavailable);
}
...
fn limited_capacity_is_reclaimed_for_the_next_image() {
    ...
    assert_eq!(first.park().unwrap(), ParkOutcome::Parked);
    assert_eq!(second.park().unwrap(), ParkOutcome::Unavailable);
    drop(first);
    assert_eq!(pool.diagnostics().free_bytes, CAPACITY);

    assert_eq!(second.park().unwrap(), ParkOutcome::Parked);
...
fn empty_images_are_not_registered_or_parked() {
    ...
    assert_eq!(image.park().unwrap(), ParkOutcome::BelowMinimum);
    assert_eq!(manager.diagnostics().image_count, 0);
...
fn minimum_size_boundary_is_inclusive() {
    ...
    assert_eq!(below.park().unwrap(), ParkOutcome::BelowMinimum);
    assert_eq!(exact.park().unwrap(), ParkOutcome::Parked);
...
fn cloned_handles_share_one_entry_and_release_only_on_last_drop() {
    ...
    assert_eq!(clone.park().unwrap(), ParkOutcome::Parked);
    drop(image);
    assert_eq!(manager.diagnostics().image_count, 1);
    assert_eq!(pool.diagnostics().free_bytes, 0);
...
fn parked_images_leave_the_resident_schedule_until_they_are_unparked() {
    ...
    assert_eq!(
        manager.force_park_images_with_report().considered,
        0,
