    ...
    N: Copy + Debug + Eq + Hash,
{
    ...
    pub(crate) fn compute_atomic_inline_layout(
        ...
        self.compute_inline_formatting_child_layout(child.to_taffy(), inputs)
    }

    /// Measure one non-replaced atomic inline-level box while retaining the
    /// dependency provenance of its inner formatting context.
    pub(crate) fn compute_atomic_inline_size(
        ...
        let inputs = self.resolve_atomic_inline_inputs(child, inputs, horizontal_margin);
        self.compute_inline_formatting_child_size(child.to_taffy(), inputs)
    }

    /// Resolve the fit-content border-box width shared by final layout and
    /// intrinsic measurement of an atomic inline-level box.
