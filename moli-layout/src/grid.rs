//! Browser-owned CSS Grid results projected from the numeric layout backend.

use style::Atom;

use crate::{LayoutGridGeometry, LayoutGridTrackGeometry};

pub(crate) fn project_grid_geometry(
    style: &taffy::Style<Atom>,
    detailed: taffy::DetailedGridInfo,
) -> Option<LayoutGridGeometry> {
...
) -> Option<LayoutGridTrackGeometry> {
...
fn expanded_line_names(
    ...
    let append_names = |line: &mut Vec<String>, names: &[Atom]| {
        line.extend(names.iter().map(ToString::to_string));
    };
...
mod tests {
    ...
    fn auto_repeat_uses_the_layout_result_count() {
        ...
        assert_eq!(expanded[1], vec!["b", "c"]);
        assert_eq!(expanded[3], vec!["e", "c"]);
        assert_eq!(expanded[11], vec!["e", "f"]);
        assert_eq!(expanded[12], vec!["g"]);
    }
    ...
    fn area_expanded_explicit_grid_does_not_change_auto_repeat_count() {
        ...
        assert_eq!(expanded[0], vec!["a"]);
        assert_eq!(expanded[4], vec!["b", "a"]);
        assert_eq!(expanded[5], vec!["b"]);
